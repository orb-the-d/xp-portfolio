...
// ── Loader setup — Draco needed for compressed GLBs ──────────────
const dracoLoader = new DRACOLoader();
// Use the CDN decoder so you don't need to host draco files yourself
...
// External callback — called when ALL models finish (success or fail)
let _onAllLoadedCb = null;
export function setOnAllLoaded(cb) { _onAllLoadedCb = cb; }

function checkAllLoaded() {
  if (loadedModels >= totalModels && totalModels > 0) {
    if (pendingModels.length > 0) revealAll();
    _onAllLoadedCb?.();
    _onAllLoadedCb = null; // fire only once
  }
}

// ── Smooth fade-in of all models together ─────────────────────────
function revealAll() {
  const STEPS    = 30;
  const INTERVAL = 20;
  let   step     = 0;
...
function loadModel(scene, path, { x=0, y=0, z=0, sx=1, sy=1, sz=1, ry=0 } = {}) {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      // Start invisible — fade in with all others at once
      model.traverse(child => {
        if (child.isMesh) {
          const mats = Array.isArray(child.material) ? child.material : [child.material];
    ...
    (err) => {
      ...
      loadedModels++; // count failures too so we don't hang forever
      checkAllLoaded();
    }
  );
}
